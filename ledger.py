...
def rjust_column(seq_of_seq_of_strings, column):
    ...
    for row in range(len(seq_of_seq_of_strings)):
        seq_of_seq_of_strings[row][column] = seq_of_seq_of_strings[row][column].rjust(
            max_len
        )
    return seq_of_seq_of_strings
...
def ljust_column(seq_of_seq_of_strings, column):
    ...
    max_len = max([len(row[column]) for row in seq_of_seq_of_strings])
    for row in range(len(seq_of_seq_of_strings)):
        seq_of_seq_of_strings[row][column] = seq_of_seq_of_strings[row][column].ljust(
            max_len
        )
    return seq_of_seq_of_strings
...
def parse_amount(amount_string):
    ...
    return {"units": "AUD", "quantity": quantity}
...
def extract_single_unit_amount(amounts):
    ...
    # else
    raise ValueError(
        "extract_single_unit_amount: amounts do not contain a single unit/ccy:", amounts
    )
...
def extract_nil_or_single_unit_amount(amounts):
    ...
    # else
    raise ValueError(
        "extract_nil_or_single_unit_amount: amounts contain >1 unit/ccy:", amounts
    )
...
def difference_nil_or_single_unit_amount(amount1, amount2):
    ...
    if amount1["units"] != amount2["units"]:
        raise ValueError(
            "difference_nil_or_single_unit_amount: different units in amount1 and amount2:",
            amount1,
            amount2,
        )
    return {
...
def ensure_balanced(transactions):
    ...
    for transaction in transactions:
        if not is_balanced(transaction):
            problem_found = True
            sys.stderr.write(
                "Line %d: Transaction does not balance. Date: '%s', description: %s.\n"
...
def ensure_date_sorted(transactions):
    ...
    for transaction in transactions[1:]:
        ...
        if transaction["date"] < last_date:
            sys.stderr.write(
                "Line %d: date: '%s' description: '%s' is not in date order.\n"
                % (transaction["line"], transaction["date"], transaction["description"])
            )
            sys.stderr.write("Exiting.\n")
            sys.exit(-1)
...
def affects(transaction_or_posting, account_string):
    ...
    if "postings" in transaction_or_posting:
        return any(
            [
                contains_account(account_string, p["account"])
                for p in transaction_or_posting["postings"]
            ]
        )
    elif "account" in transaction_or_posting:
        return contains_account(account_string, transaction_or_posting["account"])
    raise ValueError(
        "Invalid transaction_or_posting in affects:", transaction_or_posting
    )
...
def filter_by_date(transactions, first_date=None, last_date=None):
    if first_date:
        ...
        transactions = [
            t for t in transactions if dateutil.parser.parse(t["date"]) >= first_date
        ]
    if last_date:
        last_date = dateutil.parser.parse(last_date)
        transactions = [
            t for t in transactions if dateutil.parser.parse(t["date"]) <= last_date
        ]
    return transactions
...
def filter_by_account(transactions_or_postings, account_string):
    ...
    return [
        t_or_p for t_or_p in transactions_or_postings if affects(t_or_p, account_string)
    ]
...
def parse_first_line(line_number, line):
    ...
    if not is_valid_date(date_string):
        ## XXX: Should throw an exception rather than exiting completely.
        sys.stderr.write(
            "Line %d: Invalid date: '%s' in transaction '%s'\n"
...
def verify_balance(verification, account_tree, verbose):
    ...
    if extract_single_unit_amount(actual_balances) == amount:
        ...
        verify_failed = True
        sys.stderr.write(
            "FAILED: verify-balance for account '%s' at %s. Expected balance: %s. Actual balance: %s.\n"
            % (
                account_string,
                verification["date"],
                format_amount(amount),
                format_single_unit_amount(actual_balances),
            )
        )
...
def parse_balance_verify(line_number, line, adjust_sign):
    ...
    if len(split) != 4:
        sys.stderr.write(
            "Line %d: Invalid VERIFY-BALANCE operation:\n  %s\n" % (line_number, line)
        )
        sys.stderr.write(
    ...
    if not is_valid_account_string(account_string):
        sys.stderr.write(
            "Line %d: invalid account string: '%s'.\n" % (line_number, account_string)
        )
        sys.stderr.write("Exiting.\n")
        sys.exit(-1)
    ...
    return {"date": date_string, "account": account_string, "amount": amount}
...
def parse_posting(line_number, line, adjust_sign):
    ...
    return {"line": line_number, "account": account_string, "amount": amount}
...
def parse_transactions(lines, adjust_signs):
    ...
    for line in lines:
        ...
        if len(line) == 0:
            ...
            if is_balance_verify_line(line):
                verify_balances += [
                    parse_balance_verify(line_count, line, adjust_signs)
                ]
    ...
    if transaction:
        transaction["postings"] = postings
        transactions += [transaction]
    return {"transactions": transactions, "verify-balances": verify_balances}
...
AccountChartLine = namedtuple("AccountChartLine", ["name", "indent"])
...
def chart_of_accounts(accounts_dict, prefix="", indent=0):
    ...
    for account_name in account_names:
        ...
        if len(sub_accounts) == 0:
            result.append(
                AccountChartLine(name=prefix + account.original_name, indent=indent)
            )
        elif len(sub_accounts) == 1 and not account.postings:
            result += chart_of_accounts(
                account.sub_accounts, prefix + account.original_name + ":", indent
            )
        else:
            result.append(
                AccountChartLine(name=prefix + account.original_name, indent=indent)
...
def account_string_components(account_string):
    ...
    * regular with upper-cased spelling, and singular names converted
    ...
    return {"original": original, "regular": regular}


AccountTreeNode = namedtuple(
    "AccountTreeNode", ["original_name", "sub_accounts", "balances", "postings"]
)
Posting = namedtuple(
    "Posting", ["date", "amount", "account", "comment", "transaction_id"]
)
...
def _ensure_sub_account(
    account_node, sub_account_regular_name, sub_account_original_name
):
    ...
    if sub_account_regular_name not in account_node.sub_accounts:
        account_node.sub_accounts[sub_account_regular_name] = _make_account(
...
def single_unit_balances_helper(
    accounts_dict, account_names, prefix="", indent=0, print_stars_for_org_mode=False
):
    """Internal.
    ...
    if len(account_names) > 0:
        for account_name in account_names:
            ...
            result += single_unit_balances_helper(
    ...
    for account in accounts:
        ...
        if len(sub_accounts) == 0:
            result += [
                (stars, amount_string, (" " * (indent * 2)) + prefix + account_name)
            ]
        elif len(sub_accounts) == 1 and not postings:
            result += single_unit_balances_helper(
                accounts_dict[account].sub_accounts,
                [],
                prefix + account_name + ":",
                indent,
                print_stars_for_org_mode,
            )
        else:
            result += [
                (stars, amount_string, (" " * (indent * 2)) + prefix + account_name)
...
BalanceReportLine = namedtuple(
    "BalanceReportLine", ["account_name", "balance", "indent", "postings"]
)
...
def single_unit_report_helper(accounts_dict, account_names=None, prefix="", indent=0):
    ...
    if account_names:
        ...
        for account_name in account_names:
            account = find_account(account_name, accounts_dict)
            result += single_unit_report_helper(
                {account["name"]: account},
                [],
                prefix=find_original_prefix(account_name, accounts_dict),
                indent=indent,
    ...
    for account in accounts:
        ...
        if len(sub_accounts) == 0:
            ...
            result += single_unit_report_helper(
                accounts_dict[account].sub_accounts, [], "", indent + 1
            )
    return result
...
def validate_one_date_or_two(as_at_date, first_date, last_date):
    if as_at_date:
        ...
        if first_date:
            sys.stderr.write(
                "Error: first-date: %s specified in addition to as-at-date '%s'.\n"
                "Exiting." % (first_date, as_at_date)
            )
            sys.exit(-1)
        if last_date:
            sys.stderr.write(
                "Error: last-date: %s specified in addition to as-at-date '%s'.\n"
    ...
    if first_date:
        ...
        if not last_date:
            sys.stderr.write(
                "Error: first-date: %s specified, but last-date unspecified.\n"
                "Exiting." % (first_date)
            )
            sys.exit(-1)
    if last_date:
        if not first_date:
            sys.stderr.write(
                "Error: last-date: %s specified, but first-date unspecified.\n"
                "Exiting." % (last_date)
            )
            sys.exit(-1)
...
def write_excel_report(transactions, dates, output_filename):
    ...
    balances_at = {}
    for date in dates:
        balances_at[date] = single_unit_report_helper(
            calculate_balances(transactions, date)
        )
    ...
    value_font = xlwt.easyxf(
        "", '_-$* #,##0.00_-;[Red]-$* #,##0.00_-;_-$* "-"??_-;_-@_-'
    )
    txn_value_font = xlwt.easyxf(
    ...
    for date_index in range(len(dates)):
        ...
        acc_index = 0
        assert (
            len(balances_at[date]) == num_lines
        ), "Expected {} lines found {} on {}".format(
            num_lines, len(balances_at[date]), date
        )
        for line in balances_at[date]:
            ...
            if date_index == 0:
                ...
                assert (
                    account_names_dict[acc_index] == line.account_name
                ), "Expected name {} for a/c # {}. found name {}".format(
                    account_names_dict[acc_index], acc_index, line.account_name
                )
                assert (
                    indent_dict[row] == line.indent
                ), "Expected indentation {} on row {}. found {}".format(
                    indent_dict[row], row, line.indent
                )
                assert (
                    row_dict[acc_index] == row
                ), "Expected row for a/c # {} to be {}. Was {}".format(
                    acc_index, row_dict[acc_index], row
                )
    ...
    for acc_index in range(num_lines):
        ...
        for date_index in range(num_dates):
            ws.write(
                row_dict[acc_index],
                date_index,
                values_dict[(date_index, row_dict[acc_index])] * 0.01,
                value_font,
            )
        ...
        for date_index in range(1, num_dates):
            ws.write(
                ...
                (
                    values_dict[(date_index, row_dict[acc_index])]
                    - values_dict[(date_index - 1, row_dict[acc_index])]
                )
                * 0.01,
                value_font,
            )
        ...
        ws.write(
            ...
            2 * num_dates,
            (
                values_dict[(num_dates - 1, row_dict[acc_index])]
                - values_dict[(0, row_dict[acc_index])]
            )
            * 0.01,
            value_font,
        )
        ...
        for p_index in range(len(postings)):
            ...
            ws.write(row, txn_id_col + 1, postings[p_index].date, txn_text_font)
            ws.write(row, txn_id_col + 2, postings[p_index].comment, txn_text_font)
            ...
            for date_index in range(1, num_dates):
                if (
                    postings[p_index].date > dates[date_index - 1]
                    and postings[p_index].date <= dates[date_index]
                ):
                    ws.write(
                        row,
                        date_index + num_dates,
                    ...
                    ws.write(row, date_index + num_dates, 0, txn_value_font)
            ## Write "Total Difference" for final date
            if (
                postings[p_index].date > dates[0]
                and postings[p_index].date <= dates[-1]
            ):
                ws.write(
                    row,
                    2 * num_dates,
                ...
                ws.write(row, 2 * num_dates, 0, txn_value_font)
    ## Set line indent
    ...
    sys.stderr.write(
        "Wrote excel output to '{}'.\n".format(os.path.abspath(output_filename))
    )


def print_single_unit_balances(
    transactions,
    account_names,
    print_stars_for_org_mode,
    as_at_date,
    ...
    if (not first_date) and (not last_date):
        ...
        balance_text = single_unit_balances_helper(
            calculate_balances(transactions, as_at_date),
            account_names,
            print_stars_for_org_mode=print_stars_for_org_mode,
        )
    ...
    if first_date and last_date:
        ...
        first_text = single_unit_balances_helper(
            calculate_balances(transactions, first_date),
            account_names,
            print_stars_for_org_mode=print_stars_for_org_mode,
        )
        last_text = single_unit_balances_helper(
            calculate_balances(transactions, last_date),
            account_names,
        ...
        for i in range(len(first_text)):
            new_text += [
                tuple(
                    ...
                    + [
                        format_amount(
                            difference_nil_or_single_unit_amount(
                                parse_amount(last_text[i][1]),
        ...
        for line in join_columns(justify_columns(new_text, "LRRRL")):
            ...
            format_amount(
                difference_nil_or_single_unit_amount(
                    parse_amount("-$1,900.00"), parse_amount("$1,900.00")
                )
            )


def calculate_register(
    transactions, account_string, include_related_postings, first_date, last_date
):
    ...
    for transaction in transactions:
        ...
        for posting in transaction["postings"]:
            ...
            if ((not first_date) or (transaction["date"] >= first_date)) and (
                ...
                if not first_posting_output or (
                    affects(posting, account_string) and not include_related_postings
                ):
                    ...
                    description_string = ""
                if affects(posting, account_string):
                    balance_string = format_single_unit_amount(
                        find_account(account_string, account_tree).balances
                    )
                else:
                    balance_string = ""
...
def print_register(
    transactions,
    account_string,
    include_related_postings,
    reverse_print_order,
    first_date,
    last_date,
):
    data = calculate_register(
        transactions, account_string, include_related_postings, first_date, last_date
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Command-line, double-entry accounting in python."
    )
    parser.add_argument(
        "file", metavar="FILE", help="the input journal file to read from"
    )
    parser.add_argument(
        "--tweak-signs-of-input-amounts",
        default=False,
        action="store_true",
    ...
    parser.add_argument("--as-at", metavar="DATE", help="print report as-at")
    ...
    parser.add_argument(
        "--dates", metavar="DATES", nargs="+", help="print report based on these dates"
    )
    ...
    if args.generate_excel_report:
        if not args.dates or len(args.dates) < 2:
            sys.stderr.write(
                "Invalid DATES: '{}'. Need at least *two* dates..\nExiting.\n".format(
                    args.dates
                )
            )
            sys.exit(-1)
    ...
    if args.last_date:
        ...
        if args.first_date and (args.first_date > args.last_date):
            sys.stderr.write(
                "First date '%s' is after last-date: '%s'.\nExiting.\n"
                % (args.first_date, args.last_date)
            )
            sys.exit(-1)
        if args.first_date and (args.first_date == args.last_date):
            sys.stderr.write(
                "First date '%s' same as last-date: '%s'.\nExiting.\n"
                % (args.first_date, args.last_date)
            )
            sys.exit(-1)
        else:
    ...
    if args.print_transactions:
        relevant_transactions = filter_by_date(
            transactions, args.first_date, args.last_date
        )
        print_transactions(relevant_transactions)

    if args.print_balances != None:
        print_single_unit_balances(
            transactions,
            args.print_balances,
            args.print_stars_for_org_mode,
            args.as_at,
            args.first_date,
            args.last_date,
        )
    ...
    if args.print_register:
        print_register(
            transactions,
            args.print_register,
            args.include_related_postings,
            args.reverse_print_order,
            args.first_date,
            args.last_date,
        )
...
def demo():
    ...
    parsed_file = parse_file(
        "c:/Users/mafm/Desktop/working-directories/ledger.py-dev/test-data/kapsia.transactions",
        False,
    )
    transactions = parsed_file["transactions"]
    verifications = parsed_file["verify-balances"]
